...
archive_temp="$(mktemp -d /tmp/bluemap-chipped-gallery.XXXXXX)"
...
(
  ...
  LC_ALL=C find . -type f -printf '%P\n' | LC_ALL=C sort |
    zip -q -X -9 "$archive_temp/chipped-gallery.zip" -@
)
unzip -tq "$archive_temp/chipped-gallery.zip"
cp "$archive_temp/chipped-gallery.zip" "$output_path"
