...
The module contains only the sampler already used here. Chipped keeps its
emitter, Athena models, profile, route, and stock fallback. The shared class
changes only its package and API visibility. Its executable statements and
nested `Sample` record components remain unchanged.
